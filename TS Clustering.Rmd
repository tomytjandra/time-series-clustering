...
> insert equation pearson correlation
...
> insert equation euclidean distance
...
```{r, out.width = "80%", echo = FALSE, fig.align = "center", fig.cap="Difference between DTW distance and Euclidean distance (green lines represent mapping between points of time series $T$ and $S$)"}
...
But in this analysis we will only focus on two column information, namely the date of each trip `trip_start` and pickup area `picking_community_area`. 
...
In the pre-processing stage, especially in calculations similarity between data, it's a good way to normalize all the variables to bring them down to a similar scale. So we can compare two series patterns, instead of looking at their true values. To make it clear, please 
...
Before any further concept, let's compare the result of Agglomerative clustering using Euclidean versus Dynamic Time Warping. We use `agnes()` function from `cluster` package. The default linkage method is "average", we will discuss this concept later on this course. Dendogram can be plotted by specifying the parameter `which.plots = 2` inside `plot()` function. Note: we can alternatively use `hclust()` from `stats` package that should produce the same clusters.
...
The function `diana()` from the `cluster` package allows us to perform divisive hierarchical clustering. The algorithm is fully described by Kaufman and Rousseeuw (1990) [^5]. Let's compare and contrast the result of DIANA with the AGNES with complete linkage (`dtw_agnes_complete`) we've created previously:
...
How can we be sure that the chosen value of `k` or `h` is the optimal one? Fortunately, similar approach can be used to evaluate a cluster, one of them has been discussed in the previous course on Unsupervised Learning - the Elbow Method using scree plot.
...
Based on the elbow method that you've learned on the previous course, what is the optimal number of clusters `k`?
...
The highest average silhouette width is achieved when `k = 9`, but too many clusters can cause too few members in each cluster. Instead, we choose a smaller number of clusters yet the average silhouette width is closer the highest.
